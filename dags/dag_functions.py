...
TARGET_COLUMN = 'label'
MODEL_TYPE = 'LightGBM'  # or 'XGBoost', parametrize as needed
...
def check_retraining_trigger(**context):
    ...
    Triggers retraining every 2 months starting from Jun 1, 2024.
    """
    logical_date = context['logical_date']
    
    # Define retraining dates (every 2 months starting from Jun 1, 2024)
    retraining_dates = [
...
def query_mlflow_for_run_info(model_type: str = None, **context) -> Tuple[Optional[str], Optional[float]]:
    ...
    experiment_name = "loandefault_mle2"
...
def select_best_model_initial(**context):
    ...
    lightgbm_run_id = extract_mlflow_run_id_from_logs(task_id='train_lightgbm_initial_mleA2', **context)
    lightgbm_f1 = extract_metrics_from_logs(task_id='train_lightgbm_initial_mleA2', **context)
    ...
    xgboost_run_id = extract_mlflow_run_id_from_logs(task_id='train_xgboost_initial_mleA2', **context)
    xgboost_f1 = extract_metrics_from_logs(task_id='train_xgboost_initial_mleA2', **context)
...
def run_model_inference(**context):
    ...
    try:
        ...
        # Handle None values in metrics for the return statement
        auc_str = f"{comprehensive_metrics['auc']:.4f}" if comprehensive_metrics['auc'] is not None else "Not available"
        f1_str = f"{comprehensive_metrics['macro_f1']:.4f}" if comprehensive_metrics['macro_f1'] is not None else "Not available"
        
        return f"Monthly inference completed - AUC: {auc_str}, F1: {f1_str}"
...
def train_xgboost_initial(**context):
...
def process_all_bronze_tables(snapshot_date, bronze_dir):
    """
    Process all bronze tables (lms, clickstream, attributes, financials) in one function.
    """
    spark = SparkSession.builder.appName("BronzeTableProcessing").getOrCreate()
    try:
        print(f"Processing all bronze tables for date: {snapshot_date}")
        ...
        # Process LMS table
        print("Processing bronze LMS table...")
        lms_dir = os.path.join(bronze_dir, 'lms/')
        process_bronze_loan_table(snapshot_date, lms_dir, spark)
        
        # Process clickstream table
        print("Processing bronze clickstream table...")
        clickstream_dir = os.path.join(bronze_dir, 'clickstream/')
        process_bronze_clickstream_table(snapshot_date, clickstream_dir, spark)
        
        # Process attributes table (static data)
        print("Processing bronze attributes table...")
        attributes_dir = os.path.join(bronze_dir, 'attributes/')
        process_bronze_attributes_table(snapshot_date, attributes_dir, spark)
        
        # Process financials table (static data)
        print("Processing bronze financials table...")
        financials_dir = os.path.join(bronze_dir, 'financials/')
        process_bronze_financials_table(snapshot_date, financials_dir, spark)
...
def process_all_silver_tables(snapshot_date, bronze_dir, silver_dir):
    """
    Process all silver tables (lms, clickstream, attributes, financials) in one function.
    """
    spark = SparkSession.builder.appName("SilverTableProcessing").getOrCreate()
    try:
        print(f"Processing all silver tables for date: {snapshot_date}")
        
        # Import the retrofit processing function
        from silver_processing_retrofit import process_silver_table as process_silver_table_retrofit
        
        # Process all table types
        table_types = ['lms', 'clickstream', 'attributes', 'financials']
        
        for table_type in table_types:
            print(f"Processing silver {table_type} table...")
            process_silver_table_retrofit(table_type, snapshot_date, bronze_dir, silver_dir, spark)
        
        print("All silver tables processed successfully!")
        return None
    finally:
        spark.stop()

def process_all_gold_tables(snapshot_date, silver_dir, gold_dir, dpd=30, mob=6):
    """
    Process all gold tables (feature store and label store) in one function.
    """
    spark = SparkSession.builder.appName("GoldTableProcessing").getOrCreate()
    try:
        print(f"Processing all gold tables for date: {snapshot_date}")
        
        # Import the retrofit processing function
        from gold_processing_retrofit import process_gold_table as process_gold_table_retrofit
        
        # Process gold tables (feature store and label store)
        print("Processing gold tables...")
        process_gold_table_retrofit(snapshot_date, silver_dir, gold_dir, spark, dpd, mob)
        
        print("All gold tables processed successfully!")
        return None
    finally:
        spark.stop()
